...
st.title("Cronoamperometría en coordenadas cartesianas (modelo numérico C++)")
...
n_frames = st.sidebar.slider("Frames de la animación", 10, 200, 100)
...
if st.button("▶ Reproducir animación"):
    with st.spinner("Resolviendo el sistema por método numérico..."):
        times, j_vals, x_vals, profiles = solve_diffusion_implicit_1d(
            delta_x=delta_x, delta_t=delta_t, max_t=max_t
        )
